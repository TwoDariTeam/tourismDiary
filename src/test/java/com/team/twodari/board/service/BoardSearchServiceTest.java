...
class BoardSearchServiceTest {
	...
	class findByDate {
		...
		void findOrderByCreateDate() {
			...
			//when
			Slice<BoardDateDto> result = boardSearchService.findOrderByCreateDate(pageRequest);

			//then
			assertThat(result.isFirst()).isTrue();
			List<BoardDateDto> content = result.getContent();

			assertThat(content).hasSize(9);
			assertThat(content).extracting(BoardDateDto::getBoardSeq)
		...
		void findByLocationOrderByDate() {
			...
			//when
			Slice<BoardDateDto> result = boardSearchService.findOrderByCreateDateWithLocation(pageRequest, "SEOUL");
	...
	class findByPoint {
		...
		void findOrderByPoint() {
			...
			//when
			Slice<BoardPointDto> result = boardSearchService.findOrderByPoint(pageRequest);
			...
			List<BoardPointDto> content = result.getContent();

			assertThat(content).hasSize(4);
			assertThat(content).extracting(BoardPointDto::getTotalPoint)
							   .isSortedAccordingTo(Comparator.reverseOrder());
		}
		...
		void findByLocationOrderByPoint() {
			...
			assertThat(content).extracting(BoardDateDto::getTotalPoint)
							   .isSortedAccordingTo(Comparator.reverseOrder());
		}
	}
